...
class AuthorService:
    def create_author(self, body: CreateAuthorSchema) -> AuthorSchema:
    ...
    def get_author(self, author_id: int) -> AuthorSchema | None:
        return authors.get(author_id)

    def get_authors(self, body: FilterAuthorSchema) -> list[AuthorSchema]:
    ...
    def delete_author(self, author_id: int) -> int | None:
        author_id = authors.pop(author_id, None)
        if author_id:
            keys_to_remove = [
                post_id
                for post_id, post in posts.items()
                if post.author_id == author_id.id
            ]

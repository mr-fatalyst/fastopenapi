...
**A:** FastOpenAPI provides built-in security support inspired by FastAPI (but not an exact copy of its API). You can use `Security(dependency_func)` for dependency-based authentication, `SecurityScopes` for OAuth2 scopes, and pre-defined security schemes (`SecuritySchemeType.BEARER_JWT`, `SecuritySchemeType.API_KEY_HEADER`, `SecuritySchemeType.API_KEY_QUERY`, etc.) that are reflected in the OpenAPI schema once you opt in via the router's `security_scheme` parameter. See the **Security** section of the guide for examples.
...
**A:** By default FastOpenAPI generates an **OpenAPI 3.0.0** schema. You can opt into **3.1.0** by passing `openapi_version="3.1.0"` to the router (3.1 aligns with Pydantic v2's JSON Schema output and uses `type: [..., "null"]` for nullability instead of the 3.0 `nullable` keyword). Swagger UI and ReDoc both support 3.0 and 3.1 in recent versions, so the docs display correctly either way. If you encounter an issue with the documentation display, ensure you have a recent version of Swagger UI/ReDoc (the ones bundled with FastOpenAPI should be up to date).
